...
Scholar activation uses `failOnStartupError: true`. Remote authentication names a credential reference and resolves it for every request; static sensitive headers are rejected. HTTP is limited to HTTPS or explicit loopback development endpoints, and redirects are rejected. The Scholar server owns the remote corpus, embeddings, and vector indexes. Local stdio installations may use a separately installed data pack; the Scholar wheel carries code and fixed local skills rather than corpus data.
