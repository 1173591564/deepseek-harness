...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希写入启动图，并通过 `/plugins` 提供该文件及其 sourcemap。包可以通过 `dsh.client.config` 字段名允许清单发布选定的宿主插件设置；只有这些 JSON 值会进入浏览器启动图，因此仅供宿主使用的凭据和控制项不会出现其中。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
