...
| `reconnect.maxAttempts` | 两者 | 否 | 每次中断期间连续失败尝试次数上限，超出后暂停自动重试（默认 10） |
...
- 重连按中断预算控制：连续失败达到 `reconnect.maxAttempts` 次后，该服务器的工具会被注销，自动重连停止。匹配且能解析到已配置值的 Managed Credential 更新会立即启动一次使用新预算的尝试；单独删除 credential 不会启动。HMR 或重启 Host 也会创建新的 supervisor。连接存活超过 `maxDelayMs` 会重置预算，因此偶尔崩溃的服务器可以无限恢复，而崩溃循环的服务器——即使短暂连接成功——仍会耗尽上限而非永远重启。
...
- **激活后的自动重连由传输关闭触发**：崩溃的 stdio 子进程会触发重连；Streamable HTTP 失败通过每次请求以及 SDK 传输自身的 SSE（Server-Sent Events）流恢复机制暴露，因此已建立连接的 HTTP 服务器不可达时会按调用重试，而非由 supervisor 重新 spawn。初始 HTTP 连接失败和匹配且已配置的 Managed Credential 更新仍使用 supervisor generation。
