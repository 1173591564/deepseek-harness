...
远程 MCP 初始暂时不可用时，学术模式保持加载并在后台重试，不需要重启 DSH。明确的 `401`/`403` 表示 Token 无效或已撤销，需要粘贴管理员新签发的 Token；保存新 Token 会唤醒已停止的连接重试。网络故障、超时和 `5xx` 不会清除已有 Managed Credential。管理员轮换 Token 后旧值立即失效。
