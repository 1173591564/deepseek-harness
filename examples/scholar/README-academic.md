...
- `scholar_studio-0.2.6-py3-none-any.whl`：Scholar CLI、MCP server 与 15 个固定本地 skill。
...
明确的 `401`/`403` 表示 Token 无效或已撤销，需要粘贴管理员新签发的 Token。网络故障、超时和 `5xx` 不会清除已有 Managed Credential。管理员轮换 Token 后旧值立即失效。
...
```sh
python -m pip install scholar_studio-0.2.6-py3-none-any.whl
scholar init
scholar init-dsh
```
